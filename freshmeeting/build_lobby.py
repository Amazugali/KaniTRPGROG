...
FRESHMEETING_ICON_PATHS: dict[str, str] = {
    ...
    "images/2d/cb384b_32.jpg": "hm_icon.jpg",
...
def normalize_spoiler_file_value(value: str) -> str:
    """spoiler/ 起点の相対パスへ揃える。"""
    value = value.strip().replace("\\", "/")
    while value.startswith("./"):
        value = value[2:]
    if value.startswith("spoiler/"):
        value = value[len("spoiler/"):]
    return value.lstrip("/")


def derive_spoiler_title(path: Path, relative_file: str) -> str:
    """HTMLに<title>があれば優先し、無ければファイル名を表示名にする。"""
    try:
        source = path.read_text(encoding="utf-8", errors="replace")
    except OSError:
        source = ""

    match = HTML_TITLE_RE.search(source)
    if match:
...
def load_spoiler_overrides() -> list[dict[str, Any]]:
    """spoiler-rooms.json を任意の上書き設定として読む。"""
    if not SPOILER_CONFIG_PATH.exists():
        return []
    try:
        loaded = json.loads(SPOILER_CONFIG_PATH.read_text(encoding="utf-8"))
...
def load_spoiler_rooms() -> list[dict[str, Any]]:
    """spoiler/ 以下のHTMLを自動検出し、JSON設定があれば該当項目だけ上書きする。"""
    discovered: list[dict[str, Any]] = []

    if SPOILER_DIR.exists():
        for path in sorted(SPOILER_DIR.rglob("*.html"), key=lambda p: p.as_posix().lower()):
            relative_file = path.relative_to(SPOILER_DIR).as_posix()
            parent = path.relative_to(SPOILER_DIR).parent.as_posix()
            discovered.append({
                "file": relative_file,
                "title": derive_spoiler_title(path, relative_file),
                "description": (
                    "シナリオ通過者向けの感想・考察・裏話用の専用部屋。"
                    if parent == "."
                    else f"{parent} / シナリオ通過者向け専用部屋。"
                ),
                "_auto": True,
            })

    overrides = load_spoiler_overrides()
    override_by_file: dict[str, dict[str, Any]] = {}
    extra_overrides: list[dict[str, Any]] = []
    ...
    for item in overrides:
        ...
        if file_value:
            override_by_file[file_value] = item
        else:
            extra_overrides.append(item)

    result: list[dict[str, Any]] = []
    discovered_files: set[str] = set()

    for auto_entry in discovered:
        file_value = str(auto_entry["file"])
        discovered_files.add(file_value)
        merged = dict(auto_entry)
        override = override_by_file.get(file_value)
        if override:
            merged.update(override)
            merged["file"] = file_value
        result.append(merged)

    # JSONにだけ存在する未配置部屋も従来どおり「準備中」として残す。
    for file_value, item in override_by_file.items():
        if file_value not in discovered_files:
            merged = dict(item)
            merged["file"] = file_value
            result.append(merged)

    result.extend(extra_overrides)
...
def icon_dir_from(path: Path) -> str:
    """path のあるディレクトリから ROOT/img への相対URLを返す。"""
    relative = os.path.relpath(IMG_DIR, start=path.parent)
    return relative.replace("\\\\", "/")


def normalize_archive_icons(path: Path) -> tuple[int, int]:
    """人物アイコン参照を、そのHTMLの階層に応じた img/ 参照へ正規化する。

    戻り値は (Fresh Meeting 外部URLの置換数, ローカル img/ 参照の置換数)。
    再実行しても、そのHTMLの現在位置から見た正しい相対パスへ揃える。
    """
    original = path.read_text(encoding="utf-8", errors="replace")
    remote_count = 0
    icon_dir = icon_dir_from(path)
    ...
    def replace_remote(match: re.Match[str]) -> str:
        ...
        remote_count += 1
        return f'{match.group("prefix")}{icon_dir}/{icon_name}{match.group("quote")}'

    updated = REMOTE_FM_ICON_RE.sub(replace_remote, original)

    def replace_local(match: re.Match[str]) -> str:
        return f'{match.group("prefix")}{icon_dir}/{match.group("name")}{match.group("quote")}'

    updated, local_count = LOCAL_IMG_RE.subn(replace_local, updated)
...
def normalize_archive_files() -> tuple[int, int, int]:
    """lobby/ と spoiler/ 以下のHTMLを再帰的に走査する。"""
    changed_files = 0
    remote_total = 0
    local_total = 0
    ...
    paths: list[Path] = []
    if LOBBY_DIR.exists():
        paths.extend(LOBBY_DIR.rglob("*.html"))
    if SPOILER_DIR.exists():
        paths.extend(SPOILER_DIR.rglob("*.html"))

    for path in sorted(set(paths)):
        original = path.read_text(encoding="utf-8", errors="replace")
        remote_count, local_count = normalize_archive_icons(path)
...
def build() -> None:
    ...
    if spoiler_cards:
        ...
        spoiler_body = '<div class="spoiler-empty">まだ通過者専用部屋のHTMLが見つかりません。<code>spoiler/</code> 以下にHTMLを置いて再生成すると自動表示されます。</div>'
